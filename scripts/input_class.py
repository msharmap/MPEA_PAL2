...
class inputs:
    def __init__(self,input_type='PerovAlloys',input_path='/Users/maitreyeesharma/WORKSPACE/PostDoc/EngChem/HEMI/chan_code/perovs_dft_ml/',input_file='HSE_data.csv',add_target_noise = False, composition_MPEA = False):
    ...
    def read_inputs(self, verbose):
        ...
        elif self.input_type == 'MgAlloyProcessing':
            XX, YY, descriptors = self.read_MgAlloys_processing() 
    ...
    def read_perovalloys(self):
        ...
        XX = copy.deepcopy(Comp_desc.to_numpy())
        descriptors = Comp_desc.columns
    ...
    def read_MgAlloys_processing(self):
        '''
        This function reads the dataset from the COF paper: https://pubs.acs.org/doi/10.1021/acs.chemmater.8b01425
        input_type='MgAlloys_processing',
        input_path='/Users/maitreyeesharma/WORKSPACE/PostDoc/EngChem/Weihs_group_datasets/Sreenivas_MgAlloyColab_PAL2_CrankTheLoop/',
        input_file='PAL_input_training.csv'
        '''     
        data = pd.read_csv(self.filename)
        descriptors = ['Step4_ColdRolling_Strain',
                       'HT_PrimaryHT_Temperature','HT_PrimaryHT_Time',
                       'HT_SecondaryHT_Temperature','HT_SecondaryHT_Time',
                       'Step5_ColdRolling_Strain',
                       'HT_TernaryHT_Temperature','HT_SecondaryHT_Time']
        
        XX = pd.DataFrame(data, columns=descriptors)
        target = copy.deepcopy(data['Hardness_Corrosion'].to_numpy())
        YY = target.reshape(-1,1)
